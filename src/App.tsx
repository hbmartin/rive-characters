	...
	return (
		<main className="app-shell">
			<header className="gallery-header">
				<div>
					<h1>Character Animation Demos</h1>
					<p className="eyebrow">
						Use controls to interact with the animations
					</p>
				</div>
			</header>
...
function BooleanInputControl({
	...
	const value =
		typeof runtimeInput?.value === "boolean" ? runtimeInput.value : false;
...
function NumberInputControl({
	...
	const [, rerender] = useReducer((value: number) => value + 1, 0);
	const value =
		typeof runtimeInput?.value === "number" ? runtimeInput.value : 0;
		...
		<div className="number-control">
			...
			<div className="number-control-row">
				...
				<input
					...
					id={inputId}
					onChange={(event) =>
						setRuntimeValue(Number(event.currentTarget.value))
					}
